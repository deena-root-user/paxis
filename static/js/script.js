...
const devOptionsModal = document.getElementById('developer-options-modal');
const closeDevOptionsBtn = document.getElementById('close-dev-options-btn');
const saveDevOptionsBtn = document.getElementById('save-dev-options-btn');
const kaliServerUrlInput = document.getElementById('kali-server-url');
const kaliApiKeyInput = document.getElementById('kali-api-key');
const enableKaliIntegrationInput = document.getElementById('enable-kali-integration');
const mcpServerUrlInput = document.getElementById('mcp-server-url');
const mcpApiKeyInput = document.getElementById('mcp-api-key');
const enableMcpIntegrationInput = document.getElementById('enable-mcp-integration');
const enableAutonomousModeInput = document.getElementById('enable-autonomous-mode');
...
function attachStaticListeners() {
  ...
  settingsBtn.addEventListener('click', () => {
    // Settings modal is currently disabled/commented out in HTML
  ...
  devOptionsBtn.addEventListener('click', () => {
    loadSettings();
    devOptionsModal.style.display = 'flex';
  });

  closeDevOptionsBtn.addEventListener('click', () => { devOptionsModal.style.display = 'none'; });

  saveDevOptionsBtn.addEventListener('click', saveSettings);
}
...
async function handleSendMessage() {
  ...
  if (agentStatusBar) {
    agentStatusBar.className = 'status-running';
    agentStatusBar.innerHTML = '<i class="fa fa-circle-notch fa-spin"></i> Paxis is Analyzing & Executing...';
  }
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      aiMsg.innerHTML += '<br><br><em>[Agent manually interrupted by user]</em>';
    } else {
      console.error('Error during chat stream:', err);
      let errorDetails = err.message || err;

      // If the error looks like HTML (e.g. Cloudflare 524 timeout page), wrap it safely
      if (typeof errorDetails === 'string' && errorDetails.includes('<html')) {
        errorDetails = `<div style="padding: 10px; background: rgba(255,0,0,0.1); border: 1px solid #ff4444; border-radius: 5px; margin-top: 10px; max-height: 300px; overflow-y: auto;">
              <strong>Connection Error:</strong> Received an HTML response (likely a proxy/firewall error). 
              <br><br>
              <pre style="white-space: pre-wrap; font-size: 12px; color: #ff8888; background: transparent; padding: 0;">${DOMPurify.sanitize(errorDetails).replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre>
          </div>`;
        aiMsg.innerHTML = errorDetails;
      } else {
        aiMsg.innerHTML = `Error: Could not connect to the model. Detailed log: ${errorDetails}`;
      }
    }
  } finally {
    overlay.style.display = 'none';
    ...
    const agentStatusBar = document.getElementById('agent-status-bar');
    if (agentStatusBar) {
      agentStatusBar.className = 'status-idle';
      agentStatusBar.innerHTML = '<i class="fa fa-circle-check"></i> Paxis is Standby';
    }
  }
}
...
async function loadSettings() {
  try {
    ...
    if (response.ok) {
      ...
      kaliServerUrlInput.value = config.kali_server_url || '';
      kaliApiKeyInput.value = config.kali_api_key || '';
      enableKaliIntegrationInput.checked = config.enable_kali_integration || false;
      mcpServerUrlInput.value = config.mcp_server_url || '';
      mcpApiKeyInput.value = config.mcp_api_key || '';
      enableMcpIntegrationInput.checked = config.enable_mcp_integration || false;
      enableAutonomousModeInput.checked = config.enable_autonomous_mode || false;
    } else {
...
async function saveSettings() {
  const config = {
    kali_server_url: kaliServerUrlInput.value.trim(),
    kali_api_key: kaliApiKeyInput.value.trim(),
    enable_kali_integration: enableKaliIntegrationInput.checked,
    mcp_server_url: mcpServerUrlInput.value.trim(),
    mcp_api_key: mcpApiKeyInput.value.trim(),
    enable_mcp_integration: enableMcpIntegrationInput.checked,
    enable_autonomous_mode: enableAutonomousModeInput.checked
  };
  ...
  try {
    ...
    if (response.ok) {
      ...
      devOptionsModal.style.display = 'none';
    } else {
...
function buildTerminalPrompt(command) {
  ...
  const line1 =
    ...
    `<span class="t-bright"><b>User</b></span>` +
    `<span class="t-white">)</span>` +
    `<span class="t-white">-</span>` +
    `<span class="t-white">[</span>` +
    `<span class="t-bright"><b>Paxis</b></span>` +
    `<span class="t-white">]</span>\n`;
